# Application Project

**Version:** 1.1.0  
**Status:** Active  
**Updated:** 2026-04-08

---

## Keywords

`app`, `project`, `movie-cli`, `cli`

---

## Scoring

| Criterion | Status |
|-----------|--------|
| `00-overview.md` present | ✅ |
| Keywords present | ✅ |
| Scoring table present | ✅ |

---

## Purpose

Application-level project specification and documentation for the `movie-cli` CLI tool.

---

## Document Inventory

| # | File / Folder | Description |
|---|---------------|-------------|
| 01 | [01-project-spec.md](./01-project-spec.md) | Full project specification (commands, schema, TMDb, build) |
| 06 | [06-diagrams/](../06-diagrams/) | Command flow diagrams (Mermaid) — in `spec/06-diagrams/` |
| 09 | [09-app-issues/](../09-app-issues/) | Issue tracking and resolution — in `spec/09-app-issues/` |
| 97 | [97-acceptance-criteria.md](./97-acceptance-criteria.md) | GIVEN/WHEN/THEN acceptance criteria for all 15 commands |

---

## Cross-References

- [Root spec overview](../00-overview.md)
- [Error Management](../02-error-manage-spec/00-overview.md)
- [App Issues](../09-app-issues/)
